...
def create_erdos_renyi_graph(en,em):
    n = en
    m = em/float(en)
    G = nx.erdos_renyi_graph(n, m, seed=None, directed=True)
...
def iterate_shocks(G, d_nodes,phi):    
    insolvent_nodes = []
    i = 1
    shock_nodes(G, d_nodes, 0.8, phi)
    while(1):
        print 'iteration %d' %i
...
def mainCall(n,m,phi):
    G = create_erdos_renyi_graph(n,m)
    d_nodes = add_node_info(G)
    print_graph(G, 'e')
    print "---------------------"
    print_graph(G, 'n')

    iterate_shocks(G,d_nodes,phi)
